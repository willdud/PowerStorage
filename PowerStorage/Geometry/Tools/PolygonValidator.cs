...
namespace PowerStorage.Geometry.Tools
{
    public static class PolygonValidator
    {
        ...
        public static bool IsConsistent(IPolygon poly)
        {
            ...
            foreach (var seg in poly.Segments)
            {
                ...
                {
                    horrors++;
                    //logger.Warning(String.Format("Endpoints of segment {0} are coincident (IDs {1} / {2}).", i, p.id, q.id),"PolygonValidator.IsConsistent()");
                }
        ...
        public static bool HasDuplicateVertices(IPolygon poly)
        {
            ...
            for (int i = 1; i < points.Length; i++)
            {
                ...
                {
                    horrors++;
                    //logger.Warning(String.Format("Found duplicate point {0}.", points[i]), "PolygonValidator.HasDuplicateVertices()");
                }
            }
            ...
            foreach (var seg in poly.Segments)
            {
                ...
                if (q0 != null && q1 != null)
                {
                    ...
                    if (p0 == q1 && p1 != null)
                    {
                        ...
                        {
                            horrors++;
                            //logger.Warning(String.Format("Bad segment angle found at index {0}.", i), "PolygonValidator.HasBadAngles()");
                        }
                    }
                }
        ...
        private static int CheckVertexIDs(IPolygon poly, int count)
        {
            ...
            foreach (var seg in poly.Segments)
            {
                ...
                {
                    horrors++;
                    //logger.Warning(String.Format("Segment {0} has invalid startpoint.", i), "PolygonValidator.IsConsistent()");
                }

                if (q.id < 0 || q.id >= count)
                {
                    horrors++;
                    //logger.Warning(String.Format("Segment {0} has invalid endpoint.", i), "PolygonValidator.IsConsistent()");
                }
